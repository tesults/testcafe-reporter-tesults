...
module.exports = function () {  
    return {
        ...
        async reportTaskStart (/* startTime, userAgents, testCount */) {
            process.argv.forEach((val /*, index*/) => {
                if (val.indexOf(this.targetKey) === 0)
                    this.target = val.substr(this.targetKey.length + 1);
                if (val.indexOf(this.filesKey) === 0)
                    this.files = val.substr(this.filesKey.length + 1);
                if (val.indexOf(this.configKey) === 0)
                    this.config = val.substr(this.configKey.length + 1);
                if (val.indexOf(this.buildNameKey) === 0)
                    this.buildName = val.substr(this.buildNameKey.length + 1);
                if (val.indexOf(this.buildDescKey) === 0)
                    this.buildDesc = val.substr(this.buildDescKey.length + 1);
                if (val.indexOf(this.buildResultKey) === 0)
                    this.buildResult = val.substr(this.buildResultKey.length + 1);
                if (val.indexOf(this.buildReasonKey) === 0)
                    this.buildReason = val.substr(this.buildReasonKey.length + 1);
            });
            ...
            if (config !== undefined) {
                if (config[this.target] !== undefined)
                    this.target = config[this.target];
                if (this.files === undefined && config[this.filesKey] !== undefined)
                    this.files = config[this.filesKey];
                if (this.buildName === undefined && config[this.buildNameKey] !== undefined)
                    this.buildName = config[this.buildNameKey];
                if (this.buildDesc === undefined && config[this.buildDescKey] !== undefined)
                    this.buildDesc = config[this.buildDescKey];
                if (this.buildReason === undefined && config[this.buildReasonKey] !== undefined)
                    this.buildReason = config[this.buildReasonKey];
                if (this.buildResult === undefined && config[this.buildResultKey] !== undefined)
                    this.buildResult = config[this.buildResultKey];
            }
        },
        ...
        async reportTestDone (name, testRunInfo, meta) {
            ...
            if (testCase.suite === null || testCase.suite === undefined)
                delete testCase.suite;
        ...
        async reportTaskDone (/* endTime, passed, warnings */) {
            ...
            if (this.buildName !== undefined && this.buildName !== null) {
                const buildCase = { suite: '[build]' };

                buildCase.name = this.buildName;
                if (buildCase.name === '')
                    buildCase.name = '-';
                if (this.buildDesc !== undefined && this.buildDesc !== null)
                    buildCase.desc = this.buildDesc;
                if (this.buildReason !== undefined && this.buildReason !== null)
                    buildCase.reason = this.buildReason;
